...
thread_create (const char *name, int priority,
	...
	t->descriptor_table=palloc_get_multiple(PAL_ZERO, 3);
...
thread_set_priority (int new_priority) {
	if(!thread_mlfqs)
	{
		...
		*/
		...
		/*When the thread has been donated other thread's priority*/
		/*We need to only change the priority_original*/
		else
		{
...
whether_to_yield(void) {
	...
	int readyPriority=list_entry(list_begin(&ready_list), struct thread, elem)->priority;
	return currPriority>readyPriority || intr_context();
}
void
